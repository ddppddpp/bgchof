...
def calcEaster(inputYear):
    ...
    if not inputYear or inputYear < iFIRSTVALIDYEAR or inputYear > iLASTVALIDYEAR:
        return None
    #if valid
    else:
...
def main(argv):
    ...
    if (len(argv) > 2) or (len(argv) < 2):
        sys.stderr.write("USAGE: %s <year for which to calculate Easter Sunday in format YYYY> \n" % (argv[0])) 
        return None
    else:
        sInputYear = argv[1]
    ...
    try:
        ...
        sys.stderr.write("The argument should be an year for which to calculate Easter Sunday\n")
